...
from core import MODEL, resolve_model_path
...
class RoEmotionPCTool:
    def __init__(
        ...
        self.yolo = self._load_yolo(model_path, enable_yolo)
        ...
        self.yolo_status_var = tk.StringVar(
            value=f"YOLO enabled: {self.yolo_model_path.name}" if self.yolo else "YOLO unavailable; using LED blobs"
        )
    ...
    def _load_yolo(self, model_path: Path | None, enable_yolo: bool):
        if not enable_yolo:
            return None
        try:
            self.yolo_model_path = resolve_model_path(model_path or MODEL)
            return OptionalYoloDetector(self.yolo_model_path, confidence_threshold=self.confidence_threshold)
    ...
    def _session_metadata(self):
        return {
            ...
            "yolo_model_path": str(self.yolo_model_path) if self.yolo_model_path else "",
...
def main():
    ...
    parser.add_argument("--model", "--model-path", dest="model", type=Path, default=None,
                        help="Optional RoEmotion YOLO .tflite model; bundled model is used by default")
    parser.add_argument("--yolo", action="store_true", help="Compatibility flag; YOLO is already enabled by default")
    parser.add_argument("--qr-only", action="store_true", help="Disable YOLO and use QR/blob metrics only")
    parser.add_argument("--confidence", type=float, default=0.001, help="YOLO confidence threshold")
    args = parser.parse_args()

    app = RoEmotionPCTool(args.camera, args.output, args.model, args.confidence, enable_yolo=not args.qr_only)
    app.run()
